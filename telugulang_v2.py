#!/usr/bin/env python3
"""
TeluguLang v0.2 (Added if/else)
==============================

New in v0.2
------------
* `ayithe (condition) { ... } lekapothe { ... }`   # if / else
  - else block optional.
  - Works anywhere a statement is allowed.

Existing Features
-----------------
* Variable assignment:     veru x = 10
* Print (single expr):     cheppu x
* While loops:             podugu (x > 0) { ... }
* Numbers, strings, booleans (nijam/abaddam)
* Arithmetic: + - * / %
* Comparisons: == != > >= < <=
* Line comments starting with #
* Bilingual (English + తెలుగు) error messages

TODO / Coming Soon
------------------
* Multiple-arg print via commas: cheppu "X =", x
* Functions (seva)
* Telugu-script keyword aliases
* REPL mode

Usage
-----
    python telugulang.py path/to/file.tlg

Minimal if/else Example
-----------------------
veru x = 10
ayithe (x > 5) {
    cheppu "Big number!"
...
TOKEN_SPEC = [
    ...
    ("R_BRACE",     r"\}"),
    ("SEMICOLON",   r";"),
    # NOTE: COMMA not yet supported in grammar; will add later.
    ("NEWLINE",     r"\n"),
    ("SKIP",        r"[ \t\r]+"),
...
def lex(source: str) -> List[Token]:
    ...
    """
    tokens: List[Token] = []
    # Remove comments line-by-line to preserve positions.
    lines = source.splitlines(keepends=True)
    cleaned = []
    for ln, line in enumerate(lines, start=1):
        idx = line.find('#')
        if idx != -1:
            # keep newline if present so line numbers stay aligned
            if line.endswith("\n"):
                line = line[:idx] + "\n"
            else:
                line = line[:idx]
...
class Parser:
    ...
    def print_stmt(self) -> PrintStmt:
        ...
        # v0.2: still single expression. Multi-arg print coming soon.
...
class Interpreter:
    ...
    def exec_stmt(self, stmt: Node):
        ...
        elif isinstance(stmt, IfStmt):  # now fully supported
